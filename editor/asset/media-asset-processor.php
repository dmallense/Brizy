		...
		foreach ( $matches[0] as $i => $url ) {
			...
			try {
				...
				if ( $fs->has( $optimized_image_full_path ) ) {
					...
					continue;
				} elseif ( $lfs->has( $optimized_image_full_path ) ) { // try to write if localy the file is found
					$lfs->loadFileInKey( $optimized_image_full_path, $urlBuilder->upload_path( $optimized_image_full_path ) );
				...
				if ( $fs->has( $resized_image_path ) ) {
					...
					$content         = str_replace( $matches[0][ $i ], $local_media_url, $content );
					continue;
				} elseif ( $lfs->has( $resized_image_path ) ) { // try to write if localy the file is found
					$lfs->loadFileInKey( $resized_image_path, $urlBuilder->upload_path( $resized_image_path ) );
